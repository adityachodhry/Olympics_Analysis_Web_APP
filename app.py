...
if user_manu == 'Country-Wise Analysis':
    ...
    if pt.isnull().all().all():
        st.write(f"No data available for {selected_country} to generate a heatmap.")
    else:
        fig, ax = plt.subplots(figsize=(15, 15))
        ax = sns.heatmap(pt, annot=True)  # Setting a color map and vmin - vmin=0, cmap='Blues'
        st.pyplot(fig)
...
if user_manu == 'Athlete Wise Analysis':
